#ifndef ALIHFINVMASSFITTER1_H
#define ALIHFINVMASSFITTER1_H
...
/// \brief AliHFInvMassFitter1 class for the fit of
...
class AliHFInvMassFitter1 : public TNamed {
 public:

  enum ETypeOfBkg{ kExpo=0, kLin=1, kPol2=2, kNoBk=3, kPow=4, kPowEx=5};
  enum ETypeOfSgn{ kGaus=0, k2Gaus=1, k2GausSigmaRatioPar=2 };
  AliHFInvMassFitter1();
  AliHFInvMassFitter1(const TH1F* histoToFit, Double_t minvalue, Double_t maxvalue, Int_t fittypeb=kExpo, Int_t fittypes=kGaus);
  ~AliHFInvMassFitter1();
 ...
 private:
  AliHFInvMassFitter1(const AliHFInvMassFitter1 &source);
  AliHFInvMassFitter1& operator=(const AliHFInvMassFitter1& source);
  ...
  /// \cond CLASSIMP     
  ClassDef(AliHFInvMassFitter1,5); /// class for invariant mass fit
  /// \endcond
};
