#ifndef ALIHFINVMASSMULTITRIALFIT1_H
#define ALIHFINVMASSMULTITRIALFIT1_H
...
class TNtuple;
class AliHFInvMassFitter1;

/// \class AliHFInvMassMultiTrialFit1

class AliHFInvMassMultiTrialFit1 : public TNamed {
  
public:
  AliHFInvMassMultiTrialFit1();
  virtual ~AliHFInvMassMultiTrialFit1();
  ...
  void SetUseDoubleGausSignal(Bool_t opt=kTRUE){fUseDoubleGausSignal=opt;}
  ...
  void SetUseNoBackgroundOnlySignal(Bool_t opt=kTRUE){fUseNoBackgroundOnlySignal=opt;}
  ...
  void SetRangeGaus2sigma(Bool_t opt=kTRUE){ fRangeGaus2sigma=opt; }
  ...
  void SetpTMin(Double_t pt){ ptmin = pt; }
  void SetpTMax(Double_t pt){ ptmax = pt; }
  Double_t GetpTMin(){ return ptmin; }
  Double_t GetpTMax(){ return ptmax; }
  void SetMaxChi2(Double_t maxchi){ maxchisquare = maxchi; }
  Double_t GetMaxChi2(){ return maxchisquare; }
  
  void SetRawYieldRef(Double_t yield){ RawYieldRef = yield; }
  Double_t GetRawYieldRef(){ return RawYieldRef; }
  void SetSigmaRef(Double_t sigma){ SigmaRef = sigma; }
  Double_t GetSigmaRef(){ return SigmaRef; }
  
  enum EBkgFuncCases{ kExpoBkg, kLinBkg, kPol2Bkg, kPol3Bkg, kPol4Bkg, kPol5Bkg, kPowBkg, kPowTimesExpoBkg, kNoBkgOnlySig, kNBkgFuncCases};
...
private:
  ...
  AliHFInvMassMultiTrialFit1(const AliHFInvMassMultiTrialFit1 &source);
  AliHFInvMassMultiTrialFit1& operator=(const AliHFInvMassMultiTrialFit1& source);
  ...
  Bool_t fUseDoubleGausSignal; /// switch for double gaussian signal
  ...
  Bool_t fUseNoBackgroundOnlySignal; /// switch for no background, only Gaussian signal
  ...
  Bool_t   fRangeGaus2sigma;    /// flag to use 2sigma range for only Gauss fit
  ...
  TH1F* fHistoRawYieldTrialBinC0All; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldDistBinC1All; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldTrialBinC1All; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldDistBinC0All_2; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldTrialBinC0All_2; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldDistBinC1All_2; /// histo with bin counts from all trials
  TH1F* fHistoRawYieldTrialBinC1All_2; /// histo with bin counts from all trials
  ...
  TH1F** fHistoRawYieldDistBinC0_2;  /// histo with bin counts from subsamples of trials
  TH2F** fHistoRawYieldTrialBinC0_2; /// histo with bin counts from subsamples of trials
  TH1F** fHistoRawYieldDistBinC1_2;  /// histo with bin counts from subsamples of trials
  TH2F** fHistoRawYieldTrialBinC1_2; /// histo with bin counts from subsamples of trials
  ...
  Double_t fMinYieldGlobBC;   /// minimum yield
  Double_t fMaxYieldGlobBC;   /// maximum yield
  Double_t fMinYieldGlobBC_2;   /// minimum yield
  Double_t fMaxYieldGlobBC_2;   /// maximum yield
  
  Double_t ptmin;
  Double_t ptmax;
  Double_t maxchisquare;
  
  Double_t RawYieldRef;   /// Value default fit.
  Double_t SigmaRef;    /// Value default fit.
  
  std::vector<AliHFInvMassFitter1*> fMassFitters; //!<! Mass fitters
  
  /// \cond CLASSIMP
  ClassDef(AliHFInvMassMultiTrialFit1,2); /// class for multiple trials of invariant mass fit
  /// \endcond
};
