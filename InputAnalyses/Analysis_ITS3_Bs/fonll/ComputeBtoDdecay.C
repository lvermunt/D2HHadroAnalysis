...
Int_t maxentrfonll = 10; //Usually 12, but for 5.5 TeV there is no npdf uncertainty, so then = 10

void ComputeBtoDdecay(Int_t nGener=10000000,
		      Int_t pythiaver=8,
		      Int_t fonllCase=-1,
		      TString fileNameFONLLb="FONLL-Bhadron-dsdpt-sqrts5500-100GeV-50MeVbins.txt",
		      TString fileNameFONLLd0="FONLL-D0-dsdpt-sqrts5500-100GeV-50MeVbins.txt",
		      TString fileNameFONLLdplus="FONLL-Dplus-dsdpt-sqrts5500-100GeV-50MeVbins.txt",
		      TString fileNameFONLLdave="FONLL-D0DplusAv-sqrts5500-100GeV-50MeVbins.txt",
		      TString fileNameFONLLdstar="FONLL-Dstar-dsdpt-sqrts5500-100GeV-50MeVbins.txt",
		      Int_t opt4ff=1,
		      Int_t optForNorm=1,
...
TH1D* ReadFONLL(TString filename, Int_t fonllCase, Int_t nPtBins, Double_t histominpt, Double_t histomaxpt){
  ...
  while(!feof(infil)){
    ...
    for(Int_t i=0; i<maxentrfonll;i++) ok=fscanf(infil,"%f",&dum);
    if(feof(infil)) break;
    if(pt==0.) continue;
